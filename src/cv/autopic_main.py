...
from cv.autopic_lightning_model import AutopicModel
from cv.custom_wandb_logger import CustomWandBLogger
from cv.data.autopic_dataset import AutopicDataset


def train(config) -> None:
    train_manifest = pd.read_csv(os.path.join("data", "autopic_dataset", "train.csv"))
    val_manifest = pd.read_csv(os.path.join("data", "autopic_dataset", "test.csv"))
    train_ds = AutopicDataset(train_manifest, data_augmentation=True)
    eval_ds = AutopicDataset(val_manifest, data_augmentation=False)
    train_dl = torch.utils.data.DataLoader(train_ds, batch_size=64, shuffle=True)
    eval_dl = torch.utils.data.DataLoader(eval_ds, batch_size=64, shuffle=False)

    lit_model = AutopicModel(config)
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=os.path.join("runs", "autopic", config["exp_name"], "checkpoints"),  # Directory to save checkpoints
    ...
    trainer = Trainer(
        ...
        default_root_dir=os.path.join("runs", "autopic", config["exp_name"]),  # Where to save logs and checkpoints
...
if __name__ == '__main__':
    config = yaml.safe_load(open("config/autopic.yaml", "r"))
    train(config)
